...
export default ({id, firstName, lastName, age, sex, social}) => {
    ...
    if (!Array.isArray(social)) {
        throw new Error("Social should be an array")
    }

    if (social.every(({id, title, views}) => {
        return typeof id === "number" && typeof title === "string" && typeof views === "number" })) {
        throw new Error("Social should contain only objects with id as number, title as string, views as number")
    }
}
